...
4. keys
    -> key: it is used to identify the element uniquely
    -> It helps react to improve performance
    -> Mani le ke te badha elements ne key aapi che ok. Have, e badha mathi koi ek j element update or remove thayo che. 
        -> To, react, e check karse ke kayo element update/remove thayo ane e j element real DOM ma update karavase bija badha ne hath lagavya vagar.
    
        -> We can't not use index as a key
        -> Because, if any of the value from the array gots removed or entire array gets reordered, then key value for each element gets changed, which confuses the react that what is the actual key of the element.
